...
module.exports = {
	init: function (browser) {
		var wrapMethods = ['addValue','clearElement','click','doubleClick','leftClick','middleClick','moveToObject','rightClick','selectByIndex','selectByValue','selectByVisibleText','selectorExecute','selectorExecuteAsync','setValue','submitForm',
			'getHTML',
			'element'];
		_.each(wrapMethods, function(method) {
			browser[method] = _.wrap(browser[method], function (func) {
		...
		browser.dragAndDrop = _.wrap(browser.dragAndDrop, function (func) {
			var args = Array.prototype.slice.call(arguments, 1);
			return browser.lookupElementXPath(args[0]).then(function (xpath) {
		...
		browser.addCommand("lookupElementXPath", function (selector) {
			return this.execute(function (selector) {
				...
				function searchContainer(container, labels, labelIndex) {
					...
					if (lastItem) {
						...
						if (i >= 0) {
							...
							targets = targets.concat(searchContainer(childContainer, labels, labelIndex + 1));
						}
						else {
